...
exports.run = (client, message, args) => {
  ...
  let muteRole = client.guilds.get(message.guild.id).roles.find('name','Schizophren Mute');
  if (!modlog) return message.reply('`mod-log` kanalını bulamıyorum.').catch(console.error);
  if (!muteRole) return message.reply('Schizophren Mute` adlı bir rol bulamıyorum.').catch(console.error);
  if (reason.length < 1) return message.reply('Tapma sebebini yazmalısın.').catch(console.error);
