...
exports.run = (client, message, args) => {
  ...
  if (reason.length < 1) return message.reply('Sunucudan atma sebebini yazmalısın gerizekalı insan.');
  if (message.mentions.users.size < 1) return message.reply('Kimi sunucudan atacağını yazmalısın.').catch(console.error);

  if (!message.guild.member(user).kickable) return message.reply('Bu adam taşşaklıymış sunucudan atamam.');
